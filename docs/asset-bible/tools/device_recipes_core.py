...
def aux_invasion_module(seed=0):
    ...
    верш./9839 треуг., ~90KB, без цвета в исходнике). Цвет перенесён с
    более раннего высокополигонального 100%-эталона
    Aux_core_InvasionModule_2.glb (129MB, растеризован и не годится для
    прямого импорта) методом ближайшей точки: с эталона взято ~60k точек
    поверхности с цветом из базовой текстуры (с переводом linear->sRGB),
    для каждой грани референса-3 голосованием по 40 ближайшим точкам
    определён доминирующий оттенок среди узкого набора реальных цветов
    модели (tan/teal/redbright/copper/dark — остальные теги палитры
    исключены, иначе многочисленные похожие серые перетягивают грани на
    себя). Результат сохранён заранее (tools/invasion_module_mesh.npz) и
    просто грузится здесь — самого тяжёлого референса в рантайме нет.
