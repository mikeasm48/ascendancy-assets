# Референсы зданий — что сюда класть
...
> (≤1024px, jpeg q≈68..85) — их достаточно для сверки при доработке рецептов
> `tools/building_recipes_humans_*.py`. Для генерации GLB-наборов эта
> папка не требуется вовсе (см. `03-building-set.md`).

Подпапка на набор: `humans/`, `core/`, `Shuffie/`, `Bulls/`, `Worms/`.
Ниже подробно расписан набор Humans, остальные — в конце файла.

Референсы Humans лежат в подпапке `humans/`.
...
16 листов на всю застройку расы: 4 орбитальных дока, орбитальный щит,
орбитальный кинетический шар, наземный кинетик, Colony Base, фабрика,
ферма, лаборатория, жилая нора, электростанция, Science Hive Mind,
Living La Vida Loca. Кит по ним уже собран и покрашен — палитра, разбор
геометрии и известные ограничения в `../../races/worms.md`.

Корабельная часть расы — в `../ships/Worms/`; лист деталей оттуда
(`Worms_Ship_Parts.jpg`) служит эталоном палитры и для зданий тоже.

## Подпапка Bulls/

`orbital_dock_techno_bull.jpg` — единственный пока референс: фон
орбитального дока. Расы `Bulls` нет в `Race`; в ассетах у неё только
`bulls/orbital_dock/`. Корабельные референсы — в `../ships/Bulls/`.
