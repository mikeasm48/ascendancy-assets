...
Все изменения — через feature-ветку и Pull Request. Не пушить и не
коммитить напрямую в `main`.

**Единственное исключение** — `tools/release.sh`: он сам коммитит и
пушит бамп `VERSION` в main как последний шаг публикации релиза (после
того как сам контент уже неизменяемо опубликован как GitHub Release,
см. «Публикация релизов ассетов» ниже). PR для этого шага избыточен —
ревьюить нечего, релиз уже необратимо состоялся. Любой другой прямой
коммит в main (включая правки CLAUDE.md, workflow-файлов, скриптов
tools/) — только через PR.
...
**`assets/` НЕ трекается git** (см. .gitignore) — только на диске
локально. Причина: GLB-экспорт из Blender почти всегда меняется
побайтово даже для «того же» ассета (зашитые таймстемпы/метаданные
генератора), поэтому каждый релиз добавлял бы в git-историю почти
полный объём бандла (>100 МБ), а бинарники (GLB/PNG) уже сжаты и не
дельта-сжимаются packfile'ом. История `.git` за первый же релиз
доросла до ~200 МБ. Дублировать эти же байты в git бессмысленно и
опасно для размера репо: они и так навсегда живут как GitHub Release
asset (см. следующий раздел про политику хранения релизов).
...
Что она делает (см. `tools/release.sh`):
...
5. Бампает `VERSION`, коммитит и пушит в main напрямую (это триггер
   для workflow `Notify remake of new assets release`, не источник
   контента — сам контент уже в Release).
...
- `assets/` — снимок `~/.ascendancy/assets` для сборки релиза (в
  .gitignore); контент живёт только как GitHub Release asset, не в
  git-истории (см. «Публикация релизов ассетов» выше)
