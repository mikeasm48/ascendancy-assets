...
## Раскладка бандла: дизайны и маппинги

Бандл раздаёт не только модели, но и базовый маппинг «технология → что
показывать»: `common/building-designs/` и `common/device-mappings/`
(по файлу на источник: core, humans, shuffie). Игра не содержит ни
одного захардкоженного имени модели — подробности, схема файлов и
порядок поиска в `docs/asset-bible/05-design-mappings.md`.

Рядом с каждым `*_shipyard_constructor.glb` может лежать необязательный
`.json` того же имени: разворот кита по осям (worms собран «лицом» в
`+Z`, игре нужен `-Z`) и назначение узлов там, где автоклассификатор
игры ошибается. Формат и порядок приоритета (ассеты выше локального
`model-cache.json`) — в `docs/asset-bible/06-ship-constructor-metadata.md`.

Из бандла **удалены папки `buildings/` и `devices/`** — старые
одиночные модели строений и устройств, которые игра выбирала по
захардкоженным путям. Их заменяют дизайны из редакторов, а для
