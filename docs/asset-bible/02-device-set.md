...
Эти же `extras` — вход автомаппинга устройств (`common/device-mappings/`,
см. `05-design-mappings.md`): прямое совпадение по `device_id` для новых
китов и по `display_name` для core-набора, где узлы названы по артефакту,
а не по id каталога.
